...
class Fenster(QWidget): # Wichtig für Status und Menübar von QMainWindow erben
    ...
    def initMe(self):
        ...
        self.label2.setGeometry(140, 185, 150, 20)
        #Versionslink
        self.label3 = QLabel("CMD COMMAND: ", self)
        self.label3.setGeometry(15, 180, 210, 20)
        ...
        self.le1.setFont(QFont('Arial', 8))
        self.le1.setGeometry(15, 210, 230, 20)
        self.le1.setText(">> ")
        ...
        self.cb3 = QCheckBox("Use only one File", self)
        self.cb3.move(20, 150)
        self.cb3.toggle()
        self.cb3.stateChanged.connect(self.activate_load_Button)
        ...
        self.flag_icon_file = 0
        self.flag_cb2_active = 0
        self.flag_cb3_active = 0
        #################################
        # Allgmeine Fenster Config (Hauptfenster)
        #################################
        self.setGeometry(50, 50, 260, 240)
        self.setWindowTitle(f"Py2exe {ver}")
    ...
    def function_start_conversion(self):
        ...
        if self.flag_load_file: # Wenn Datei ausgewählt wurde wird funktion freigeschaltet
            ...
            if self.fileName[0]:
                ...
                if cb2_check == 2: #2 for checked state
                    self.flag_cb2_active = 1
                    print(f"   Option 2: CMD in Exe deaktiviert")
                cb3_check = self.cb3.checkState()
                if cb3_check == 2: #2 for checked state
                    self.flag_cb3_active = 1
                    print(f"   Option 3: Programm in ein File aktiv")

                # Optionen mithilfe von Bitmasks übergeben
                opt_arg = 0
                opt_arg = self.flag_icon_file | opt_arg        # -> XXXX XXX1 Wenn Icon geladen
                opt_arg = self.flag_cb2_active << 1 | opt_arg  # -> XXXX XX1X -> Wenn CMD ausgeschaltet
                opt_arg = self.flag_cb3_active << 2 | opt_arg  # -> XXXX X1XX -> Wenn OneFile gewünscht
                ...
                if opt_arg & 0b0000100:  # Wenn CMD Window deaktiviert werden soll für die EXE
                    arg_3 = f" --onefile"
                else:
                    arg_3 = ""
                ...
                command_2 = fr'pyinstaller {arg_3}{arg}{arg_2} {filename}'
