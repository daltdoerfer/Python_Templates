...
filenames = glob(r"C:\Users\altdoerfer\Desktop\20220601_e1v1u22d0000tx_VC09.SP00.00\*.cdfx")
...
<V>0.</V>
               ...
               'dummyIDATA_L2mp': '''<SHORT-NAME>L2mp_IDATA.dummyIDATA_L2mp</SHORT-NAME>''',
               'dummyIDATA_L2mw': '''<SHORT-NAME>L2mw_IDATA.dummyIDATA_L2mw</SHORT-NAME>'''}
...
for filename in filenames:
    ...
    for (key, search_text) in search_dict.items():
        ...
        with io.open(filename, mode="r", encoding="utf-8") as textfile:
            ...
            if contains(search_text, textfile):
                print(rf"    OK: {key} exakt enthalten")
            else:
                print(rf"    MISSING KEY: {key} nicht gefunden")
                errorflag += 1

    if errorflag == 0:
        print(rf"--------------------------------------------------")
        print(rf"CHECK PASSED: {textfile.name}")
        print(rf"--------------------------------------------------")
    else:
        print(rf"--------------------------------------------------")
        print(rf"CHECK FAILED !")
        print(rf"Please check File {textfile.name} manually !")
        print(rf"--------------------------------------------------")